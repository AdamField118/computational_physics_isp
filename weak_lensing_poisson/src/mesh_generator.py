...
def generate_p2_structured_mesh(nx: int, ny: int,
                                xmin: float = 0.0, xmax: float = 1.0,
                                ymin: float = 0.0, ymax: float = 1.0,
                                return_numpy: bool = False):
    """
    Generate P2 structured triangular mesh
    
    Process:
    1. Create P1 mesh (vertices only)
    2. Add edge midpoint nodes
    3. Build 6-node element connectivity
    
    Args:
        nx, ny: Number of cells in x, y directions
        xmin, xmax, ymin, ymax: Domain bounds
        return_numpy: Return numpy arrays instead of JAX
        
    Returns:
        Mesh object with P2 elements (6 nodes per triangle)
    """
    from .fem_solver import Mesh
    
    # Step 1: Create base P1 mesh to get vertex positions and connectivity
    p1_mesh = generate_structured_mesh(nx, ny, xmin, xmax, ymin, ymax, return_numpy=True)
    
    # Step 2: Create edge midpoint nodes
    # We need to track which edges we've already created midpoints for
    edge_to_midpoint = {}  # (i, j) -> midpoint_node_index
    nodes_list = list(p1_mesh.nodes)  # Start with vertex nodes
    
    # Helper to get or create edge midpoint
    def get_edge_midpoint(n1: int, n2: int) -> int:
        """Get midpoint node index for edge (n1, n2), creating if needed"""
        edge_key = tuple(sorted([n1, n2]))  # Canonical edge ordering
        ...
        if edge_key not in edge_to_midpoint:
            ...
            midpoint_idx = len(nodes_list)
            nodes_list.append(midpoint)
            edge_to_midpoint[edge_key] = midpoint_idx
        
        return edge_to_midpoint[edge_key]
    
    # Step 3: Build P2 element connectivity
    p2_elements = []
    
    for elem in p1_mesh.elements:
        v0, v1, v2 = elem  # Vertex nodes
        
        # Get edge midpoint nodes
        m01 = get_edge_midpoint(v0, v1)
        m12 = get_edge_midpoint(v1, v2)
        m20 = get_edge_midpoint(v2, v0)
        
        # P2 element: [vertices, then edge midpoints]
        p2_elem = [v0, v1, v2, m01, m12, m20]
        p2_elements.append(p2_elem)
    
    # Step 4: Identify boundary nodes (vertices + midpoints on boundary)
    ...
    if not return_numpy:
        ...
        p2_elements = jnp.array(p2_elements, dtype=jnp.int32)
        boundary = jnp.array(boundary)
    else:
        p2_elements = np.array(p2_elements, dtype=np.int32)
    
    print(f"P2 mesh created:")
    print(f"  Vertex nodes: {len(p1_mesh.nodes)}")
    print(f"  Edge midpoints: {len(edge_to_midpoint)}")
    print(f"  Total nodes: {n_nodes}")
    print(f"  Elements: {len(p2_elements)} (6 nodes each)")
    
    return Mesh(nodes=nodes_array, elements=p2_elements, boundary=boundary)


def visualize_p2_element():
    """
    Visualize a P2 element showing node numbering
    
    Useful for understanding the node ordering
    """
    ...
    # All nodes
    nodes = np.array([v0, v1, v2, m01, m12, m20])
    labels = ['v₀', 'v₁', 'v₂', 'm₀₁', 'm₁₂', 'm₂₀']
    ...
    for i, (node, label) in enumerate(zip(nodes, labels)):
        if i < 3:  # Vertices
            ...
            plt.plot(node[0], node[1], 'bs', markersize=10)
        
        # Label with offset
        offset = 0.08
        if i == 0:  # v0
            plt.text(node[0] - offset, node[1] - offset, label, fontsize=14, ha='right')
        elif i == 1:  # v1
            plt.text(node[0] + offset, node[1] - offset, label, fontsize=14, ha='left')
        elif i == 2:  # v2
            plt.text(node[0] - offset, node[1] + offset, label, fontsize=14, ha='right')
        elif i == 3:  # m01
            plt.text(node[0], node[1] - 2*offset, label, fontsize=14, ha='center')
        elif i == 4:  # m12
            plt.text(node[0] + offset, node[1] + offset, label, fontsize=14, ha='left')
        elif i == 5:  # m20
            plt.text(node[0] - 2*offset, node[1], label, fontsize=14, ha='right')
    
    plt.xlim(-0.2, 1.2)
    plt.ylim(-0.2, 1.2)
    plt.axis('equal')
    plt.grid(True, alpha=0.3)
    plt.title('P2 Triangle Element\n6 Nodes: 3 Vertices (red) + 3 Edge Midpoints (blue)', 
              fontsize=14)
    plt.tight_layout()
    plt.savefig('p2_element_nodes.png', dpi=150)
    print("Saved P2 element visualization to: p2_element_nodes.png")
    plt.close()


# Example usage
if __name__ == "__main__":
    print("Testing P2 mesh generation...")
    
    # Visualize single element
    visualize_p2_element()
    
    # Create small P2 mesh
    p2_mesh = generate_p2_structured_mesh(3, 3, xmin=0, xmax=1, ymin=0, ymax=1)
    
    print(f"\nP2 mesh stats:")
    print(f"  Nodes: {p2_mesh.n_nodes}")
    print(f"  Elements: {p2_mesh.n_elements}")
    print(f"  Boundary nodes: {len(p2_mesh.boundary)}")
    print(f"  DOF per element: {p2_mesh.elements.shape[1]}")
